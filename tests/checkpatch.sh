...
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
           08b9e449e94e99c0ec924e2026dc650b63ddc3e5)" > \
...
./scripts/checkpatch.scm \
    -c 08b9e449e94e99c0ec924e2026dc650b63ddc3e5 \
    -f \
    manual/gnuboot.texi > \
...
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
            5303fab6a49796d69fc9176b715409f925d8f608)" > \
...
./scripts/checkpatch.scm \
    -c 3cfb105dbc9dff5040be5ef461e14c6c1f8dace3 \
    -f \
    manual/gnuboot.texi > \
...
################################################################################
# Test if checkpatch.scm correctly finds colon (':') in node names             #
################################################################################

# This checks the "manual: GRUB images: add information about RAID
# partitions." commit which adds node names with colons.
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
            b228639da2d3700014cf21f5e36ea7d9dddc0132)" > \
    "${tmpdir}"/checkpatch-test-nodes-with-colon.log 2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when trying to detect node with colon (':') in the manual."
...
# This checks the "manual: flash programmers: add Chipflasher v2."
# commit which add nodes names without colon.
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
            1dfdcec2f34517912f2e6c29c5e4fc30209679ff)" > \
    "${tmpdir}"/checkpatch-test-nodes-without-colon.log \
    2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when checking for node without colon (':') in the manual."
...
################################################################################
# Test if checkpatch.scm correctly finds comma (',') in node names             #
################################################################################

# This checks the "manual: Add section about flash programmers."
# commit which adds comma names with colons.
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
           352aac31b493a1db4af1665e3df511c05d41d811)" > \
    "${tmpdir}"/checkpatch-test-nodes-with-comma.log 2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when trying to detect node with comma (',') in the manual."
...
# This checks the "manual: flash programmers: add Chipflasher v2."
# commit which add nodes names without comma.
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
           1dfdcec2f34517912f2e6c29c5e4fc30209679ff)" > \
    "${tmpdir}"/checkpatch-test-nodes-without-comma.log \
    2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when checking for node without colon (':') in the manual."
...
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir} "\
           4c46963c3dd2a9f26fb48363a5b7c8395be61f5a)" > \
...
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
           1f07a190bab3fb06c6adca70d72355a149ee09b3)" > \
...
./scripts/checkpatch.scm \
    -f \
    -c 5a4ea74643d1014d420d01a59de0d89d50847d8f \
...
################################################################################
# Test if checkpatch.scm correctly finds copyright headers in files            #
################################################################################

# shellcheck disable=SC2046
./scripts/checkpatch.scm -f $(git ls-files) \
    1>"${tmpdir}"/checkpatch-unhandled-copyright-headers.log 2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when trying to detect unhandled copyright headers."

grep \
    "ERROR: missing copyrights in" \
    "${tmpdir}"/checkpatch-unhandled-copyright-headers.log | \
    sed 's#^.\{29\}##g' | \
    sort -u > \
...
echo "[ OK ] detected known unhandled copyright headers."

################################################################################
# Test checkpatch.scm with a commit(s) that contains a patch(es).              #
################################################################################

# The commit c18e78555d120568062554cdaebbe780f95c6073
#  ("ressources/coreboot/fam15h*: fix building crossgcc 8.3.0 (fixes
#  #64870)") can't be used here as the Author doesn't match the
#  signed-off-by.

# This checks the "wrappers: add Guix wrapper and use it." commit
# which modifies scripts/checkpatch.scm without updating the
# copyright.
./scripts/checkpatch.scm \
    "$(git format-patch -1 -o "${tmpdir}" \
           f633e7597ed7d2f26687bf1e079d2b62dde5cf3a)" > \
    "${tmpdir}"/checkpatch-test-missing-copyright-update.log 2>/dev/null || \
    fail "checkpatch.scm failed " \
         "when trying to detect missing copyright update in a patch."
...
patches="$("${topdir}"/resources/wrapper/git format-patch -o "${tmpdir}" \
    639f71f6e92407aafa03d7c364215c2f162367e1)"
